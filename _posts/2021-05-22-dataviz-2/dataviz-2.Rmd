...
* **Purpose:** To have a clear overview on the trade volume comparison between imports and exports of Singapore and the 10 trading partners between a fixed time period of 2011 to 2020.
...
For this first chart, we will be examining the net exports and imports trade value based on each individual countries, using a diverging lollipop chart.

1. Open up a fresh empty worksheet, and drag _Variables_ into _Rows_ and _Filters_ pane. Because the data source includes the total exports and imports by all global regions, we will filter it out to only visualize the 10 trading markets. Unselect the total exports and imports before clicking on _Apply_.
...
2. Next, we will create a calculated field to show the net trade values. Right-click onto the Data pane and click on _Create Calculated Field_. We will use an if-else function to multiply the SGD value to negative if it its imports (aka sheet = 'T1'). Hence, we can apply the formula as shown in Fig 4.3.3 and rename it to "Trade Value Indicator".


<center>

![Figure 4.3.2: Filtering Variables](img/4.3.2.png){width=50%}

![Figure 4.3.3: Filtering Variables](img/4.3.3.png){width=75%}
