...
#bgmap <- st_read(dsn = "data/Geospatial", layer = "Abila")
...
gps_point <- gps_sf %>%
  group_by(line) %>%
  summarize(m=mean(datetime),  # for group by purpose , no meaning at all,just to summary
            do_union=FALSE) %>%
  st_cast("POINT")


p = npts(gps_point, by_feature = TRUE)
gps_point <- cbind(gps_point, p)


gps_point <- left_join(gps_point,subset(mb3,select=c("line","message")),by=c("line"="line"))


gps_point<-gps_point%>% mutate_if(is.character, ~gsub('[^ -~]', '', .)) # remove characters non UTF-8
...
tm_shape(bgmap) +
  tm_rgb(bgmap, r=1, g=2, b=3,
         ...
         max.value = 255)+
  tm_shape(gps_point)+
  tm_bubbles(size=0.05, style = "pretty")
