...
# cc_loc2$geo <- "-"
# for (i in 1:nrow(cc_loc2)){
#   for (j in 1:nrow(sf_mpsz)){
#     if(cc_loc2$fetype[i]==sf_mpsz$FETYPE[j] &
#        cc_loc2$street[i]==sf_mpsz$FENAME[j] &
#        cc_loc2$dir[i]==sf_mpsz$FEDIRP[j] &
#        cc_loc2$num_round[i]==sf_mpsz$FROM[j]) {
#       
#       cc_loc2$geo[i] <- sf_mpsz$geometry[j]
#     }
#     
#   }
# }

# merging data --> note that not all will match!
cc_loc3 <- right_join(cc_loc2, sf_mpsz,
                     by = c("fetype" = "FETYPE",
                            "street" = "FENAME",
                            "dir" = "FEDIRP",
                            "num_round" = "FROM"),
...
ggplot() + 
  ...
  geom_point(data = tweet_summary, aes(x = longitude, 
                 ...
                 size = msg_count,
                 colour = factor(id))) +
  geom_sf(data = cc_loc4$geometry, size = 0.8, color="darkred") +
  xlab("Longitude") +
  ylab("Latitude") +
  guides(col=guide_legend("Distinct Locations"),
         size=guide_legend("Microblog Counts")) +
