...
The table below shows the important R packages that will be used in this challenge.

| Package | Description |
|:-------|:-------------|
| tidyverse | Used in data wrangling techniques  |
| dplyr | Provided operaters/functions for data wrangling techniques |
| ggplot2 | Basic package in plotting various types of graphs |
| plotly |  To create interactive graphical visuals |
| lubridate | To convert and deal with unstructured datetime |
| ggraph | To plot network graphs  |
| igraph | Similar to ggraph |
| DT | To provide interactive data tables |
| tm | Providing functions to pre-process words  |
| wordcloud | Visualizing words |
| tidytext | To pre-process words |
| sf | Loading of Geospatial shapefiles. |
| leaflet | To provide interactivity in maps |
| crosstalk | Used together with leaflet package, require the latest version for HTML non-dependencies as if not there will be some issues |
...
### 4.1 Brief overview of events by Hashtags
...
- **7-7:30pm:** The most frequent hashtag is "dancingdolphinfire", and this suggest a fire broke out in a place called Dancing Dolphin.
...
### 5.1 Topic Modeling by Cosine Similarity and Walktrap Algorithm
...
### 5.2 Sentiment Analysis (Risk Level)
...
### 5.3 Time-series Analysis of Topics and Sentiment
...
| 3 | 2 incidents - Fire at Dancing Dolphin building and a possible hit-and-run |From 6:45pm onwards (fire), and 7pm onwards (hit-and-run)|
...
Because of the unstructuredness in the street addresses given in ccdata, some addresses cannot be matched properly. Hence, the code below will try to match the remaining street names / orphan lines that does not have numbers or directions
...
### 6.3 Combining both mbdata and ccdata
...
In addition, we can refer to the table denoting the estimated timeline for each topic, and hence filter the markers based on the time period of interest. We are also interested in knowing the topics related to the 4 major events - Fire, shooting, hit-and-run and rally. Hence, topics 2 and 4 will be removed from analysis, while topics 5, 7 and 8 will be merged together with 4 events since they are related.

By comparing the location of mmdata and ccdata, a brief compiled findings is shown in the table below:

| Topic No. | Description | Timeline Estimate from Microblog | First Call Centre Timing (wrt to Microblog Location) |
|:---:|:------------------|:------------| :-------------| 
| 1, 7, 8 | Shooting standoff at a gelato place | From 7:15pm onwards | 7:39pm |
| 3, 5, 7, 8 | Fire at Dancing Dolphin building  | From 6:45pm onwards | 6:40pm |
| 3, 7, 8 | A possible hit-and-run | From 7pm onwards | 7:18pm |
| 6 | POK rally | Fluctuates from 5-6pm and 7-8pm | 6:04pm |



## 7. Conclusion and Suggestions

Upon deeper analysis, we noticed that all 3 major events; Fire, hit-and-run and shooting standoff are actually interlinked and happened in that corresponding order.

From the geospatial and timeline results, we noticed that the call timings received by the emergency dispatches are not relative to the estimated starting time of each events. The only on-time call for assistance received is from the Fire at Dancing Dolphin, and we can see there are several call reports following the incident. This indicates the severity of the situation, which is validated by the sudden increase in fear sentiment from 6:45pm onwards, the approximate starting point of the Fire. 

In comparison to the other events, the reports made were about 10-20 minutes later, and this might be due to chaos in the situation especially for the hit-and-run as well as the shooting standoff. As the fire was the first to occur, emergency responders (fire department, police department and paramedics) should be dispatched there first (which in this case they responded promptly). The only difference that I would recommend is for them to cordon off entire area as well as dispatch more officers to the scene for crowd controlling as well as surveying. From there, the suspicious black van can be sighted faster and when the hit-and-run occurs, there are actually police nearby the scene to assist the victim or pursuit. This might help to prevent the shooting event that occur later if the black van has been captured prompty.
