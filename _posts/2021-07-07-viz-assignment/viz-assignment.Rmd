...
output:
  distill::distill_article:
    #code_folding: Click to see code
...
packages = c('lubridate','tidyverse', 'ggplot2', 'ggraph', 'igraph', 'lsa', 'DT',
             ...
             'TSstudio','hrbrthemes','plotly', 'ggridges','dygraphs','tokenizers',
             'data.table','leaflet','crosstalk')
...
#   cosine_sim <-  acos(similarity) * 180 / pi 
#   return(1 - cosine_sim)
# }
# 
# 
# msg_tfidf <- TFIDF(mb2$words.with.HT)
# 
# # 3. calculate pair-wise distance matrix 
# #pr_DB$set_entry( FUN = Cosine, names = c("Cosine") )
# d1 <- dist(msg_tfidf, method = "euclidean")
...
Based on boxplot, filter threshold of TF-IDF > = 4
...
# abila <- st_read('data/Geospatial/Abila.shp',quiet=TRUE)
...
separating street names to geocoded version to get geometry
...
Note --> 128 rows are matched only to shape file and some addresses are duplicate but have different lat/long
...
<br/>
Plotting static graph showing the volume in microblogs 

```{r echo=FALSE, fig.width=10}
# library(geosphere)
# cc_loc3$long <- "-"
# cc_loc3$lat <- "-"
# for (i in 1:nrow(cc_loc3)){
#   geo <- unlist(cc_loc3$geometry[i])
#   lo <- as.numeric(as.character(geo[geo<30]))
#   la <- as.numeric(as.character(geo[geo>=30]))
#   f <- do.call(rbind,Map(data.frame, LONG=lo, LAT=la))
#   if (length(f)==2){
#     centre <- midPoint(c(f$LONG[1],f$LAT[1]),c(f$LONG[2], f$LAT[2]))
#   } else {
#     centre <- centroid(f)
#   }
#   
#   cc_loc3$long[i]<-centre[1]
#   cc_loc3$lat[i]<-centre[2]
# }
...
combo <- ggplot() + 
  ...
  geom_point(data = tweet_summary, aes(x = longitude, 
                 ...
                 size = msg_count,
                 colour = factor(id))) +
  geom_sf(data = cc_loc3$centre_geom, shape = 23, fill = "red") +
  xlab("Longitude") +
  ylab("Latitude") +
...
Interactive plot showing call-type and topics of microblog at location
...
cc_loc3$lat <- "-"
for (i in 1:nrow(cc_loc3)){
  cc_loc3$long[i] <- unlist(cc_loc3$centre_geom[i])[1]
  cc_loc3$lat[i] <- unlist(cc_loc3$centre_geom[i])[2]
}


leaflet(data = cc_loc3) %>% 
  addTiles() %>%
  addPolygons(data = Kronos_small, weight = 4, col = 'black') %>%
  addPolygons(data = shapeData, weight = 2, col='black') %>%
  addMarkers(lng = ~as.numeric(cc_loc3$long),
             lat = ~as.numeric(cc_loc3$lat),
             popup = ~as.character(cc_loc3$message)) %>%
...
Finding out each event starting time >>> map to tweets timing >>> map to call centre timing
...
Filtering with time/topic?

```{r echo=FALSE}

# # finding columns that are different and fill with NA
# cc_loctime[setdiff(names(mb_loctime2), names(cc_loctime))] <- NA
# mb_loctime2[setdiff(names(cc_loctime), names(mb_loctime2))] <- NA
...
#filter_slider("datetime","Time", sd, column = ~datetime, timeFormat = "%T", timezone = "+0000")

bscols(
  # filter_slider("datetime","Time", 
  #               sd, column = ~datetime, 
