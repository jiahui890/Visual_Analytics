...
cc_loc4 <- cc_loc3[!is.na(cc_loc3$type),]

# cc_loc4$long <- "-"
# cc_loc4$lat <- "-"
# for (i in 1:nrow(cc_loc4)){
#   geo <- unlist(cc_loc4$geometry[i])
#   cc_loc4$long[i] <- as.numeric(mean(geo[geo<30]))
#   cc_loc4$lat[i] <- as.numeric(mean(geo[geo>=30]))
# }
...
for (i in 1:nrow(cc_loc4)){
  ...
  cc_loc4$long[i]<-centre[1]
  cc_loc4$lat[i]<-centre[2]
}
...
ggplot() + 
  ...
  geom_point(data = tweet_summary, aes(x = longitude, 
                 ...
                 size = msg_count,
                 colour = factor(id))) +
  geom_sf(data = cc_loc4$geometry, size=1, color="darkred") +
  xlab("Longitude") +
  ylab("Latitude") +
  guides(col=guide_legend("Distinct Locations"),
         size=guide_legend("Microblog Counts")) +
...
# cc_loc2 %>%
#   separate(location2, c("number","st"), sep = "(?<=[0-9]) ", convert=T)
# 
# 
# 
# sf_mpsz %>%
#   unite(street, FEDIRP:FENAME, sep = "")
# 
# 
# sf_mpsz %>%
#   inner_join(cc_loc2, by=c("FROM"="num_round",
